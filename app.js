...
function updateWeekLabel(){
  ...
  weekLabel.textContent=`Tuần ${currentWeekStart.toLocaleDateString()} - ${e.toLocaleDateString()}`;
}
window.prevWeek=()=>{currentWeekStart.setDate(currentWeekStart.getDate()-7);updateWeekLabel();loadTasks();}
window.nextWeek=()=>{currentWeekStart.setDate(currentWeekStart.getDate()+7);updateWeekLabel();loadTasks();}

/******** LOGIN ********/
window.login=()=>{
  const u=username.value.trim().toLowerCase(),p=password.value.trim();
...
window.addEventListener("DOMContentLoaded",()=>{
  const u=localStorage.getItem("user"),r=localStorage.getItem("role");
  if(u&&r)initApp(u,r);
});
...
function initApp(u,r){
  ...
  updateWeekLabel();renderHeader();loadTasks();loadHistory();
}

/******** TASK ********/
window.addTask=()=>{
  if(currentRole!=="admin")return;
  const name=nameInput.value.trim(),day=dayInput.value,text=taskInput.value.trim(),time=timeInput.value;
  if(!name||!day||!text||!time)return alert("Nhập đủ thông tin");
  const [hour,minute]=time.split(":").map(Number);
  db.collection("tasks").add({
    name,day,text,time,hour,minute,
    ...
    days.forEach(day=>{
      ...
      (data[name][day]||[]).forEach(t=>{
        ...
        if(currentRole==="admin"&&t.done){
          ...
          del.onclick=()=>confirm("Xóa nhiệm vụ?")&&db.collection("tasks").doc(t.id).delete();
          div.appendChild(del);
        }
...
function loadHistory(){
  db.collection("history").orderBy("time","desc").onSnapshot(s=>{
...
window.deleteHistory=id=>currentRole==="admin"&&db.collection("history").doc(id).delete();
window.clearHistory=()=>{
  if(currentRole!=="admin"||!confirm("Xóa toàn bộ lịch sử?"))return;
  db.collection("history").get().then(s=>s.forEach(d=>d.ref.delete()));
};
